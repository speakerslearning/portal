...
function openReviewModal(questionNumber) {
    ...
    if (questionNumber >= 37 && questionNumber <= 40) {
        // Part 6 - Reality TV commentators
        modalTitle.innerHTML = 'Select a commentator for question <span id="modalReviewNumber">' + questionNumber + '</span>';
        modalBody.innerHTML = `
        ...
        // Part 8 - Museum work
        modalTitle.innerHTML = 'Select a person for question <span id="modalReviewNumber">' + questionNumber + '</span>';
        modalBody.innerHTML = `
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'A')">
                <strong>A</strong> Melanie Kershaw
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'B')">
                <strong>B</strong> Angela Garcia
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'C')">
                <strong>C</strong> Laura Johansson
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'D')">
                <strong>D</strong> Yasmin Kumar
            </div>
        `;
    }
...
function selectReview(questionNumber, letter) {
    ...
    if (questionNumber >= 37 && questionNumber <= 40) {
        ...
        // Part 8 - Show letter with person name
        const names = {
            'A': 'Melanie Kershaw',
            'B': 'Angela Garcia',
            'C': 'Laura Johansson',
...
function selectSentence(letter) {
    ...
    // Get the full paragraph text - CAE005 Studying art history article
    const paragraphs = {
        'A': 'Burton concurs: \'Had I wanted to work in an auction house, curate or write for an art magazine, then I would have been very well placed, especially if I\'d continued on with a master\'s degree, and there were some tempting options at the Courtauld, such as art restoration. But history of art led me into writing, specifically journalism, and achieving a good degree from a prestigious university did my job prospects no harm at all.\'',
        'B': 'However absorbing or enriching history of art may be, this view of it as being accessible to and useful for only a small and probably privileged group of people can deter many with a genuine interest in it.',
        'C': 'Although it has a limited following, it is anything but insignificant in scope or academic rigour, however. It\'s about art, of course, but it\'s by no means confined to staring at paintings and stroking one\'s chin.',
        'D': '\'If you\'ve never formally studied the subject before, you start with a leveller course,\' she says. This introduction provides an overall perspective on the degree. \'You then move on to a more in-depth look, and the chance to learn about things you\'re personally interested in.\'',
        'E': 'In fact, anyone who wants to be seriously involved in museum and gallery curation needs to ensure they have a solid academic grounding in the subject. You don\'t get to run a major gallery without a deep knowledge of the art world as a whole – one you can\'t just get from avidly attending exhibitions.',
        'F': 'Another recent history of art graduate agrees: \'It gave me the opportunity to explore the stories within works of art, addressing social, political or personal reasons behind their particular appearance or fame.\'',
        'G': '\'We try to look for someone who shows a demonstrable interest in the subject – but that doesn\'t mean they have to know reams about art history. Those coming to history of art from other disciplines bring something different and enliven the discipline for everyone.\''
    };
