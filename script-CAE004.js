...
function openReviewModal(questionNumber) {
    ...
    if (questionNumber >= 37 && questionNumber <= 40) {
        // Part 6 - Newspaper analysts
        modalTitle.innerHTML = 'Select an analyst for question <span id="modalReviewNumber">' + questionNumber + '</span>';
        modalBody.innerHTML = `
        ...
        // Part 8 - Modern composers
        modalTitle.innerHTML = 'Select a composer for question <span id="modalReviewNumber">' + questionNumber + '</span>';
        modalBody.innerHTML = `
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'A')">
                <strong>A</strong> Mark-Anthony Turnage
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'B')">
                <strong>B</strong> Michael Berkeley
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'C')">
                <strong>C</strong> Sally Beamish
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'D')">
                <strong>D</strong> David Sawer
            </div>
        `;
    }
...
function selectReview(questionNumber, letter) {
    ...
    if (questionNumber >= 37 && questionNumber <= 40) {
        ...
        // Part 8 - Show letter with composer name
        const names = {
            'A': 'Mark-Anthony Turnage',
            'B': 'Michael Berkeley',
            'C': 'Sally Beamish',
            'D': 'David Sawer'
        };
...
function selectSentence(letter) {
    ...
    // Get the full paragraph text - CAE004 Kenya wildlife article
    const paragraphs = {
        'A': 'They have steadily built a fine base for exploring, and we saw more than we could have hoped: graceful giraffe, endangered Grevy\'s zebra, large families of elephant, no less than four species of eagle and, of course, the rhino. Alas, we were not to see a big cat as the grass was high after the rains, and they proved elusive.',
        'B': 'As the sun dipped behind the hills at the end of each fantastic day, however, we were able to enjoy the serenity of the wilderness, along with enthralling conversation about Kenyan wildlife and culture.',
        'C': 'It\'s so disturbing it hardly bears thinking about, but when we were offered the chance to talk to the security team about their work, we took it. They treated us to a demonstration: from the scent of a single footprint, a specially trained dog expertly tracked a \'poacher\' through the scrub, an exercise that has helped the team make hundreds of arrests.',
        'D': 'We steadily approached, and as the animal treated our arrival calmly, we could work out which it was. Sadly, the few of these remarkable creatures we were lucky enough to see are part of a declining population – though, fortunately, there are many people battling to save them.',
        'E': 'Perhaps because the landscape was drier and flatter, we saw even more wildlife here than in the previous reserve, more than making up for the interrupted sleep. Swathes of warthog were an obvious addition, along with many jackal and a solitary hyena.',
        'F': 'Joseph came to collect us there, and needlessly explained that we would see much more than rhino; either side of the runway gathered zebra, a couple of giraffe and an ostrich before our very eyes.',
        'G': 'Besides this income diversification, there is a successful campaign to demonstrate that grazing cattle can regenerate grasslands and give back to the wildlife. Among the animals here are four of only seven remaining northern white rhino worldwide. They are in an enclosed reserve with added security, their horns trimmed to be less enticing to poachers.'
    };
