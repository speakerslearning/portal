...
function updateProgressBar() {
    const totalSections = 6; // We have 6 parts total for C1
...
function finishTest() {
    // Calculate scores
    let totalScore = 0;
    let totalQuestions = 56; // 8 + 8 + 8 + 12 + 12 + 8
...
function calculateTotalScore() {
    ...
    let score = part1Score + part2Score + part3Score + part4Score + part5Score + part6Score;
    let total = 56; // 8 + 8 + 8 + 12 + 12 + 8
...
// Part 6 Modal Functions for Cross-text Matching
function openReviewModal(questionNumber) {
    ...
    // Update onclick handlers for all review choices
    const choices = document.querySelectorAll('#reviewModal .student-choice');
    choices.forEach(choice => {
        const letter = choice.querySelector('strong').textContent;
        choice.onclick = function() {
            selectReview(questionNumber, letter);
        };
    });
}
