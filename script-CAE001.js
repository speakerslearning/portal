...
function openReviewModal(questionNumber) {
    ...
    if (questionNumber >= 37 && questionNumber <= 40) {
        ...
        // Part 8 - Student entrepreneurs
        modalTitle.innerHTML = 'Select a student for question <span id="modalReviewNumber">' + questionNumber + '</span>';
        modalBody.innerHTML = `
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'A')">
                <strong>A</strong> Lisa James
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'B')">
                <strong>B</strong> Joanna Payne
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'C')">
                <strong>C</strong> Alexis Taylor
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'D')">
                <strong>D</strong> Zoe Roberts
            </div>
        `;
    }
...
function selectReview(questionNumber, letter) {
    ...
    if (questionNumber >= 37 && questionNumber <= 40) {
        ...
        // Part 8 - Show letter with student name
        const names = {
            'A': 'Lisa James',
            'B': 'Joanna Payne',
            'C': 'Alexis Taylor',
...
function selectSentence(letter) {
    ...
    const paragraphs = {
        'A': 'To some extent, however, the Maya may have shaped their own decline. At the height of the civilisation, the area had a population of tens of millions, and vast numbers of trees would have been cut down to make space for cities and farms, and to provide fuel.',
        'B': 'Indeed, it was during the rainiest decades of this era, from AD 440 to 660, that the cities sprouted. In fact, all the hallmarks of Maya civilisation — sophisticated political systems, monumental architecture, complex religion — came into full flower during this era.',
        'C': 'One common misconception is that the Maya completely vanished after the arrival of the Spanish in the sixteenth century. There are, in fact, Mayan people still in the area today, with their own culture and traditions. The mystery, historically, is why their cities were abandoned and never reclaimed.',
        'D': 'With their pyramids, squares, ball courts and government buildings, they were where the urban Maya discussed philosophy, developed an accurate solar-year calendar and relished the world\'s first hot chocolate. Farmers, too, were riding high, turning hillsides into terraced fields to feed the fast-growing population.',
        'E': 'Add this to the broader trend and the situation becomes dire — a cautionary tale for the modern world, some warn. The fear is that, as more and more forestland is turned into farms and cities, and as global temperatures continue to rise, we may risk the same fate that befell the Maya.',
        'F': 'The Maya religious and political system was based on the belief that rulers were in direct communication with the gods. When these divine connections failed to produce rainfall and good harvests, tensions probably developed. In only 25 years between 750 and 775, for example, 39 rulers commissioned the same number of stone monuments — evidence of \'rivalry, war, and strategic alliances,\' according to Kennett\'s study.',
        'G': 'Today, however, scientists generally agree that the Maya collapse has many roots, all intertwined — overpopulation, warfare, famine, drought. And one of the hottest fields of inquiry in recent years has centred on climate change, perhaps of the Maya\'s own doing.'
    };
