...
function calculateScorePart4(startQ, endQ) {
    ...
    for (let i = startQ; i <= endQ; i++) {
        ...
        const fullAnswer = correctAnswerObj.full.toUpperCase().trim();
        const part1Answer = correctAnswerObj.part1.toUpperCase().trim();
        const part2Answer = correctAnswerObj.part2.toUpperCase().trim();
        ...
        if (userAnswer === fullAnswer) {
            ...
            if (userAnswer.includes(part1Answer)) {
                score += 1;
            }
            if (userAnswer.includes(part2Answer)) {
                score += 1;
            }
        }
    }
...
function createPartResult(partName, score, total, partKey, startQ, endQ) {
    ...
    if (partKey === 'part4') {
        for (let i = startQ; i <= endQ; i++) {
            ...
            const fullAnswer = correctAnswerObj.full;
            const part1Answer = correctAnswerObj.part1.toUpperCase().trim();
            const part2Answer = correctAnswerObj.part2.toUpperCase().trim();
            ...
            // Check scoring
            if (userAnswer === fullAnswer.toUpperCase().trim()) {
                pointsEarned = 2;
                statusIcon = '✓✓';
                statusClass = 'correct';
            } else {
                let hasPart1 = userAnswer.includes(part1Answer);
                let hasPart2 = userAnswer.includes(part2Answer);
            ...
            detailsHTML += `
                <div class="question-detail ${statusClass}">
                    ...
                    <span class="answer-info">
                        ...
                        ${pointsEarned < 2 ? ` | Correct answer: <strong>${fullAnswer}</strong>` : ''}
                    </span>
                </div>
            `;
        }
