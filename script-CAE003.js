...
function openReviewModal(questionNumber) {
    ...
    if (questionNumber >= 37 && questionNumber <= 40) {
        ...
        // Part 8 - Record label founders
        modalTitle.innerHTML = 'Select a person for question <span id="modalReviewNumber">' + questionNumber + '</span>';
        modalBody.innerHTML = `
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'A')">
                <strong>A</strong> Ben Wolf
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'B')">
                <strong>B</strong> Henry Stone
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'C')">
                <strong>C</strong> Kerry Murphy
            </div>
            <div class="student-choice" onclick="selectReview(${questionNumber}, 'D')">
                <strong>D</strong> Riz Amal
            </div>
        `;
    }
...
function selectSentence(letter) {
    ...
    // Get the full paragraph text - CAE003 sports testing article
    const paragraphs = {
        'A': 'The first of these uses a large board full of bulbs, which light up in a random sequence. You hit them to turn them off, while simultaneously shouting out the numbers that appear on a small screen. I do better than the average, which I put down to being used to dealing with two small children.',
        'B': 'For all the cutting-edge equipment and carefully designed assessments, what Dr Carter is unable to evaluate, as he readily admits, are such crucial factors as \'pain tolerance, doing the training day in, day out and self-belief.\'',
        'C': 'Looking at someone\'s genome to determine whether they\'re suited to team sports or individual endurance may be \'accessible in the next decade,\' Dr Carter says. \'Say we\'ve got a group of 15-year-olds who are genetically suited to certain sports, are we going to put them through special regimes for the next decade? That\'s where some of the research is going.\'',
        'D': 'After measuring my handgrip strength (I\'m delighted to be \'average\', given my arms are like spaghetti), it\'s time for the final hurdle: the Wingate test. Or as I shall always think of it, the longest half minute of my life. It sounds relatively easy.',
        'E': 'Disappointingly, it turns out I\'ve been kidding myself about my height for years. Weight, though, is less important than body composition and it\'s the latter that the first of a series of hi-tech tests will assess.',
        'F': 'So much so that hours later I can still feel the lactic acid, brought on by the extreme pedalling, burning in my legs. \'Sprinters, cyclists – this is their test,\' explains Dr Carter. Top athletes \'need to be performing well on this or there\'s something wrong.\'',
        'G': 'While I am doing this, Dr Carter records my data. One of the key indicators, he explains, is \'the respiratory exchange ratio, which is the combination or the ratio of oxygen consumed and carbon dioxide produced. When that ratio goes over one, that\'s when we know to increase the gradient.\''
    };
